...
func TestParseBDIssues_QuestionTitle(t *testing.T) {
	...
	// close_reason is preferred over description
...
// fakeEmbedder returns a pre-set vector for each text (by index).
...
func TestCluster_GroupsNearDuplicates(t *testing.T) {
	// q1 and q3 are near-identical (same vector); q2 is different.
	sources := []Source{
	...
	// Cluster 0: the two "search" questions
	if len(clusters[0]) != 2 {
		t.Errorf("cluster[0] length = %d, want 2", len(clusters[0]))
	}
	// Cluster 1: BM25 question alone
	if len(clusters[1]) != 1 {
