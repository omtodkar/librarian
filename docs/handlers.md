...
| Rust | `.rs` | function, method (fn inside `impl`), class (struct), enum, interface (trait), type (type alias); `impl Trait for Type` scopes methods under the type path; `mod` blocks scope nested items; **trait super-bounds** (`trait Foo: Bar`) emit `inherits` with `relation=extends`; `use` declarations with `::` separator, grouped `{A, B}`, aliased `as X`, and wildcard `*` imports; pub/private items both indexed |
| Ruby | `.rb` | class, module (as class), method, singleton_method (`def self.foo`), field (attr_accessor/attr_reader/attr_writer — multi-symbol calls expanded via PostProcess); `require`/`require_relative`/`load` imports; `require_relative` paths resolved to project-relative file paths anchored at the file's directory; `include`/`extend`/`prepend` emit `inherits` with `relation=mixes`; class `< Superclass` emits `inherits` with `relation=extends` |
