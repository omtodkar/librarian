...
### Added
- **Multiple docs directories** (`doc_dirs` array configuration) for polyrepo / multi-repo workspaces: index the union of multiple documentation directories as one knowledge base, with workspace-root-relative stored paths to disambiguate same-named files across sub-repos (e.g. `proto/docs/DOCTRINE.md` vs `astro-engine/docs/DOCTRINE.md`). Backward compatible: `docs_dir` (singular string) is retained as a deprecated alias. `librarian init` auto-detects sub-repo `docs/` directories and confirms the list with the user (interactive) or writes them without prompting (non-interactive/CI), with an optional `--doc-dir` flag override.
- **MCP tool enhancement:** `capture_session` gained an optional `docs_dir` parameter to target one of the configured docs directories (defaults to the primary/first); allows session notes to be filed into the appropriate sub-repo's docs tree in multi-repo workspaces.
